...
class PrepareTransactionActorsTest < ActiveSupport::TestCase
  ...
  test "returns sender and recipient with name from slack in array" do
    ...
    recipient = PrepareRecipient.new(slack_adapter).call(team.slack_team_id, service_params, 'valid')
  ...
  test "returns recipient with sanitized name with dots" do
    ...
    recipient = PrepareRecipient.new(slack_adapter).call(team.slack_team_id, service_params.merge({text: '+1 name.with.dots..'}), 'valid')
    assert_equal('name.with.dots', recipient.slack_user_name)
  end

  test "returns recipient with sanitized name with url format" do
    slack_adapter = SlackAdapter.new
    recipient = PrepareRecipient.new(slack_adapter).call(team.slack_team_id, service_params.merge({text: '+1 <http://asd.com|asd.com>'}), 'valid')
